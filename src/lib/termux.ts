...
export const TERMUX_STORAGE_HINT = 'Ejecuta termux-setup-storage para habilitar el almacenamiento compartido (~/storage/download/Downlink)'
// relative to ~/storage — download/Downlink is a dedicated shared-storage
// folder for downloads (browser file manager, not the gallery)
export const TERMUX_VIDEOS_DIR = path.join('download', 'Downlink')
...
/**
 * Download destination: ~/storage/download/Downlink (a dedicated shared
 * storage folder) when termux-setup-storage ran (created on first run),
 * ~/Downloads otherwise. Desktop always ~/Downloads.
 */
